...
func newMinioHandler(bucket string, expireDays TimeDays, config *MinioConfig) (DFSHandler, error) {
...
func (h *minioHandler) DelObject(path, fileName string) error {
	ctx, cf := context.WithTimeout(context.Background(), time.Second*10)
	defer cf()
	if path[len(path)-1] != '/' {
		path += "/"
	}
	err := h.minioClient.RemoveObject(ctx, h.bucket, path+fileName, minio.RemoveObjectOptions{})
	if err != nil {
		return err
	}
	return nil
