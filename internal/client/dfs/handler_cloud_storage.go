...
func newCloudStorageHandler(bucket string, expireDays TimeDays, config *CloudStorageConfig) (DFSHandler, error) {
...
func (h *cloudStorageHandler) DelObject(path, fileName string) error {
	ctx, cf := context.WithTimeout(context.Background(), time.Second*10)
	defer cf()
	...
	err := h.cloudStorgeClient.Bucket(h.bucket).Object(path + fileName).Delete(ctx)
	return err
}
