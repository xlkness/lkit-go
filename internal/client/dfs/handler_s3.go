...
func newS3Handler(bucket string, expireDays TimeDays, config *S3Config) (DFSHandler, error) {
...
func (h *s3Handler) DelObject(path, fileName string) error {
	ctx, cf := context.WithTimeout(context.Background(), time.Second*10)
	defer cf()
	...
	_, err := h.s3Client.DeleteObjectWithContext(ctx, input)
	if err != nil {
		return err
	}
	return nil
