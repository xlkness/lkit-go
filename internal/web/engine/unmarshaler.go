...
var structuredUnmarshaler = func(c *gin.Context, unmarshalHandler unmarshal, structTemplate interface{}) (string, string, interface{}, string, string, error) {
	...
	if err != nil {
		log.Errorf("read io error:%v", err)
		return c.Request.RequestURI, "", nil, "", "", err
	}
	if len(buf) == 0 { // 如果body没有参数，则参数来自url
		for i := 0; i < to.NumField(); i++ {
			...
			if err != nil {
				...
				return c.Request.RequestURI, "", nil, field.String(), fieldStr, fmt.Errorf("query param(%v) error:%v", fieldTagName, err)
			}
		}
	} else { // 如果body有参数，则用body的参数反序列化
		var err error
		if unmarshalHandler == nil {
			err = json.Unmarshal(buf, receiver)
		} else {
			err = unmarshalHandler(buf, receiver)
		}
		if err != nil {
			return c.Request.RequestURI, string(buf), receiver, "", "", fmt.Errorf("unmarshal error:%v", err)
		}
	}
	return c.Request.RequestURI, string(buf), receiver, "", "", nil
}
