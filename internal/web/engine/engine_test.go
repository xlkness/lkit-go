...
func (c *MyContext) ResponseParseParamsFieldFail(path string, url, body, field string, value string, err error) {
			...
			<head>
				...
				<div align="center">
					...
					{{ range $idx, $path := .Paths }}
						<tr>
						<th colspan="4" style="background-color: green">{{ $path.Route.ShortDesc }}</th>
						</tr>
						<tr>
						<td>请求路径</td>
						...
						{{ range $idx1, $field := $path.Route.JsonStructDesc }}
							...
							<td>{{printf "%s" $field.ShortDesc }}</td>
							</tr>
						{{ end }}
						{{ else }}
