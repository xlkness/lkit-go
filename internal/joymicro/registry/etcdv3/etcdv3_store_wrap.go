...
func (p *EtcdV3RegisterPlugin) Start() error {
	if p.Expired == 0 {
		// 2倍刷新时间+2秒冗余
		p.Expired = p.UpdateInterval*2 + time.Second*2
	}
	...
	if p.kv == nil {
		kv, err := newV3Store(p.EtcdServers, p.Options)
		if err != nil {
	...
	err := p.kv.Put(p.BasePath, []byte("rpcx_path"), &store.WriteOptions{IsDir: true, TTL: p.Expired})
				...
				case <-ticker.C:
					...
					for _, name := range p.Services {
						...
						if err != nil {
							...
							err = p.kv.Put(nodePath, []byte(meta), &store.WriteOptions{TTL: p.Expired})
							if err != nil {
							...
							p.kv.Put(nodePath, []byte(v.Encode()), &store.WriteOptions{TTL: p.Expired})
						}
					}
				}
			}
...
func (p *EtcdV3RegisterPlugin) Register(name string, rcvr interface{}, metadata string) (err error) {
	...
	if p.kv == nil {
		kv, err := newV3Store(p.EtcdServers, nil)
		if err != nil {
			log.Errorf("cannot create etcd registry: %v", err)
			return err
		}
	...
	nodePath = fmt.Sprintf("%s/%s/%s", p.BasePath, name, p.ServiceAddress)
	err = p.kv.Put(nodePath, []byte(metadata), &store.WriteOptions{TTL: p.Expired})
	if err != nil {
		log.Errorf("cannot create etcd path %s: %v", nodePath, err)
		return err
	}
